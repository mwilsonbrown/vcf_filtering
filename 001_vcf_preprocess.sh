...
RAW_VCF=/mnt/scratch/wils1582/BIG_vcfs/merged_final_CBP2_CR_CG_on_CBP-CG.v.vcf
...
bcftools query -l $RAW_VCF > pre_samples.txt

# Run R script to generate files for subsetting vcf
Rscript 001a_check_raw_vcf.R pre_samples.txt "$PREFIX"

# reneame samples
bcftools reheader -s "$OUTDIR"/"$PREFIX"_new_names.txt $RAW_VCF -Ov -o "$PREFIX"_rmF2.vcf
