...
# Maya Wilson Brown
#
# Goal: Filter Capsella orientalis on CG subgenome while keeping invairant sites to later join with other Capsella species
#
######################## SETUP
# Variables
...
## then, dump sites with more than 10% missing calls and Quality under 20
bcftools filter -e 'QUAL="." || FMT/DP<3' --set-GTs . "$PREFIX"_filter1.vcf.gz -Ou | \
  bcftools view -i 'F_MISSING<0.1' -e 'QUAL<20' \
